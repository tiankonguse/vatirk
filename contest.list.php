...
while($result && $row=mysql_fetch_array($result)) {
	...
	if( $status == $status_contest_end || 
		$status == $status_contest_will_do ||
		$status == $status_mycontest_end ||
		$accept
		){
		mysql_query("delete from contest where  oj = '$row_oj' and name = '$row_name' and id != '$row_id'" ,$conn);
...
?>
	...
	<ul class="pager">
		<li class="previous <?php if($start_page == 0){echo "disabled";}  ?>">
			<a href="?status=<?php echo $status;?>&start_page=<?php echo ($start_page-1); ?>">&larr; previous</a>
		</li>
		<li class="next <?php if($start_page == $page_count){echo "disabled";}  ?>">
			<a href="?status=<?php echo $status;?>&start_page=<?php echo ($start_page+1); ?>">next &rarr;</a>
		</li>
	</ul>
</div>
