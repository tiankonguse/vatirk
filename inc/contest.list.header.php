...
$sql_head = "select * from vatirk_contest";
...
if ($username != "") {
	$sql_array [$status_contest_will_do] = "$sql_head where start_time + duration >= '$now_time' and id in (select contest_id from vatirk_contest_user where user_id = '$user_id')";
	$sql_num ++;
	
	$sql_array [$status_mycontest_end] = "$sql_head where start_time + duration < '$now_time' and id in (select contest_id from vatirk_contest_user where user_id = '$user_id')";
	$sql_num ++;
}
