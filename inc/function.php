...
function update_acmicpc() {
	...
	if ($json) {
		...
		foreach ( $contest_objs as $contest_obj ) {
			...
			$start_time = gettime ( $start_time );
			save ( $oj, $name, $start_time, $link, 18000, $cid);
...
function save($oj, $name, $start_time, $link, $duration, $cid) {
	...
	$oj = mysql_real_escape_string ( trim($oj) );
	$name = mysql_real_escape_string ( trim($name) );
	$start_time = mysql_real_escape_string ( $start_time );
	$cid = mysql_real_escape_string ( $cid );
	
	$sql = "select * from vatirk_contest where  oj = '$oj' and  cid = '$cid'";
	$result = mysql_query ( $sql, $conn );
	...
	if ($result && mysql_num_rows ( $result ) > 0) {
		...
		if ($duration != $row ['duration']) {
			$sql = "update `vatirk_contest` SET `duration` = '$duration' where id = '" . $row ['id'] . "'";
			mysql_query ( $sql, $conn );
		}
	} else {
		$sql = "insert into vatirk_contest (name, link, oj, start_time, duration, cid) values('$name','$link','$oj','$start_time','$duration','$cid')";
// 		var_dump($sql);
		$result = mysql_query ( $sql, $conn );
	}
}
